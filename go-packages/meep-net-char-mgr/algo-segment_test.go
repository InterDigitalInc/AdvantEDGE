...
func TestSegAlgoStatic(t *testing.T) {
	...
	if flow, ok := algo.FlowMap["zone1-fog1-iperf:ue1-iperf"]; ok {
		if flow.Path == nil || len(flow.Path.Segments) != 3 {
			t.Errorf("Flow has invalid path")
		}
		log.Error(printPath(flow.Path))
	} else {
		t.Errorf("Failed to find flow")
	}
	if flow, ok := algo.FlowMap["zone2-edge1-iperf:ue1-iperf"]; ok {
		if flow.Path == nil || len(flow.Path.Segments) != 7 {
			t.Errorf("Flow has invalid path")
		}
		log.Error(printPath(flow.Path))
	} else {
	...
	if flow, ok := algo.FlowMap["zone1-fog1-iperf:ue1-iperf"]; ok {
		log.Error(printFlow(flow))
	}
	if flow, ok := algo.FlowMap["zone2-edge1-iperf:ue1-iperf"]; ok {
		log.Error(printFlow(flow))
	}

	fmt.Println("Test algo calculation with metrics")
	...
	if !validateNetCharUpdate(updatedNetCharList, "zone1-fog1-iperf", "ue1-iperf", 0, 0, 0, 1000) {
	...
	if flow, ok := algo.FlowMap["zone2-edge1-iperf:ue1-iperf"]; ok {
		if flow.Path == nil || len(flow.Path.Segments) != 5 {
			t.Errorf("Flow has invalid path")
		}
		log.Error(printPath(flow.Path))
	} else {
	...
	// Validate algorithm Calculations
	if !setMetrics(rc, "zone1-fog1-iperf", "ue1-iperf", 0.1) {
		t.Errorf("Error updating metrics")
	}
	if !setMetrics(rc, "zone2-edge1-iperf", "ue1-iperf", 100) {
		t.Errorf("Error updating metrics")
	}
	...
	if !setMetrics(rc, "zone2-edge1-iperf", "ue1-iperf", 19.3) {
		t.Errorf("Error updating metrics")
	}
	updatedNetCharList = algo.CalculateNetChar()
	if flow, ok := algo.FlowMap["zone1-fog1-iperf:ue1-iperf"]; ok {
	...
	if !setMetrics(rc, "zone2-edge1-iperf", "ue1-iperf", 19.9) {
		t.Errorf("Error updating metrics")
	}
	updatedNetCharList = algo.CalculateNetChar()
	if flow, ok := algo.FlowMap["zone1-fog1-iperf:ue1-iperf"]; ok {
	...
	if !setMetrics(rc, "zone2-edge1-iperf", "ue1-iperf", 0.5) {
		t.Errorf("Error updating metrics")
	}
	// if !setMetrics(rc, "zone1-fog1-svc", "ue2-svc", 100) {
	// 	t.Errorf("Error updating metrics")
	...
	// 	t.Errorf("Invalid net char update list")
	// }
	if !validateNetCharUpdate(updatedNetCharList, "zone1-fog1-iperf", "ue1-iperf", 0, 0, 0, 23) {
		t.Errorf("Error in Net Char update")
	}
	...
	if !setMetrics(rc, "zone1-fog1-iperf", "ue1-iperf", 0.2) {
		t.Errorf("Error updating metrics")
	}
	...
	if !setMetrics(rc, "zone1-fog1-iperf", "ue1-iperf", 10.1) {
		t.Errorf("Error updating metrics")
	}
	if !setMetrics(rc, "zone2-edge1-iperf", "ue1-iperf", 0) {
		t.Errorf("Error updating metrics")
