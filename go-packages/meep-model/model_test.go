...
func TestGetSetActiveElements(t *testing.T) {
	...
	respDeployment := m.GetDeployment(&filter)
	if respDeployment == nil {
		t.Fatalf("Failed to get expected deployment")
	}
	...
	//failure path
	fmt.Println("Get Deployment")
	respDeployment = m.GetDeployment(&badFilter)
	if respDeployment != nil {
		t.Fatalf("Failed to get expected deployment")
	}
...
func TestValidateScenario(t *testing.T) {
	...
	ValidatorVersion = semver.Version{Major: 1, Minor: 8, Patch: 2}
	...
	fmt.Println("Validate scenario: scenarioVer[1.8.2] == validatorVer[latest]")
	validJsonScenario, status, err = ValidateScenario([]byte(testScenario_v1_8_2), "")
