...
func NewRouter() *mux.Router {
	...
	// Service Api files
	handler = http.StripPrefix("/mon-engine/v1/api/", http.FileServer(http.Dir("./api/")))
	router.
		PathPrefix("/mon-engine/v1/api/").
		Name("Api").
		Handler(handler)
	// User supplied service API files
	handler = http.StripPrefix("/mon-engine/v1/user-api/", http.FileServer(http.Dir("./user-api/")))
	router.
		PathPrefix("/mon-engine/v1/user-api/").
		Name("UserApi").
		Handler(handler)
